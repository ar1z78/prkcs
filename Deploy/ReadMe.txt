...
ClickSaver 2.7.5 Updated by Ar1z for Project Rubi-Ka
=====================================================
...
Additionally, it can search for substrings in item names and can
...
On first run, it asks for AO directory and connects to the items database.
For PRK it is the subfolder "\client" inside the folder where
you have installed the PRK Client. In this subfolder exist
Anarchy.exe and AnarchyOnline.exe.

Usage
-----

The "missions" tab shows various data for each mission (Playfield,
coordinates, QL, reward item, find item, item icon and cash)
Items matching the "item watch" criteria are displayed in red.
In addition, a status bar at the bottom can display additional info.

The "Search" tab shows an item list where you can enter text to
search among item names. Ex: "battle suit", "hand axe", etc.

Item search allows for web-search-like constructors "<text>", -<text> and
word-match.
Examples:
...
The "location list" is the same as the "item list", but
...
shortcuts to edit the lists:

 - Arrow keys: select entry
 - Return: add entry
 - Shift+return or double click: edit entry
 - DEL: remove entry
The "Mission Type" lets you select a type of mission you wish to be alarmed on.

The "Settings" tab:
Options:
 - Start Minimized: if this is checked, ClickSaver start in minimized state.
 ...
 - Sounds: This turns on/off the sounds.. It plays found.wav and notfound.wav
   when running buying agent.
 - Select Match: This turns on/off the mouse movement to select the first
   matching mission when using buying agent
...
enter the maximum tries and the numbers of missions you want to
roll, and you just hit start. The buying agent will get new missions
until either the maximum tries or the missions have been reached.
...
anymore and that it will move the mouse around every time you generate
...
Note that AO had to be closed whenever CS wanted to create a new items
database. The creation of an intermediate DB was removed in v2.7.0

This versions no longer use winpcap. It hooks on to a function
in a dll of the game to get mission information.
It means that it's now completely independent of your network
...
https://github.com/pzychotic/ClickSaver
...
2.7.0       - Removed the dependency to Berkley DB, Now it connects directly to PRK Database
...
2.6.2		- Fixed the bug with Return Item missions (Project RK had changed it's id)
			...
			- Added text in in the select PRK folder to make it easier to understand which folder it is.
...
2.3.0 beta 2    - Should work with 15.2.0 now
                - Added option called "Auto Expand Team Mission" should be self explanatory
...
2.2.9 beta  - Fixed a bug with 14.8 where missions without rewards would stop clicksaver from reading the other missions after it
            - Added Mission XP reward to display
                ...
                - Fullscreen mode - If you hit this, it starts a mode where it will automatically start buying agent every time you roll the first mission. It also has a "Fullscreen mode" UI box that never switches windows, which should get rid of some window swapping issues
...
2.2.4       - Sets a big (32Mb) cache for Berkeley database when creating the local database.
        ...
        - Processes the windows messages while duplicating the database so that task manager no longer says that clicksaver isn't answering while
...
2.2.3 - Fixed for 14.4.
    ...
    (playfield info, items, and icons). It take much less space on hard
    ...
    It's no longer necessary in 14.4 because the request button now
    enables itself automatically.
    Additionally, buying agent now move the mouse over the request button
...
2.2.1 - Adapted for 14.2: item infos in database are no longer crypted,
    ...
    time and not skip over the matching mission (I consistently
...
1.1.4 - Fixed another problem of non-freed gdi objects in PUL. Was
    causing display to get screwed up completely on win98 after the
